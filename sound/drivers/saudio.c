					     ...
					     size_t size, size_t max)
{
	...
	for (stream = 0; stream < SAUDIO_STREAM_MAX; stream++) {
		for (substream = pcm->streams[stream].substream; substream;
			...
			uint32_t addr = smem_alloc(size);

			if(addr) {
			    dmab->dev.type = type;
			    dmab->dev.dev = data;
			    dmab->area = ioremap_nocache(addr, size);
			    dmab->addr = addr;
			    dmab->bytes = size;
			    memset(dmab->area, 0x5a, size);
			    pr_debug
				("saudio_pcm_lib_preallocate_pages_for_all:saudio.c: dmab addr is %x, area is %lx,size is %d",
				 (uint32_t) dmab->addr, (unsigned long)dmab->area,
				 size);
