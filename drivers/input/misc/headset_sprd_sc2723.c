...
static int adc_get_average(int gpio_num, int gpio_value)
{
    ...
    struct sprd_headset *ht = &headset;
    struct sprd_headset_platform_data *pdata = ht->platform_data;
    PRINT_DBG("ADC_CHANNEL_HEADMIC:0x%x", ADC_CHANNEL_HEADMIC);
    //================= debug =====================
    if(debug_level >= 3) {
        int count = 0;
        int adc_val = 0;
...
static struct sprd_headset_platform_data *headset_detect_parse_dt(struct device *dev)
{
    ...
    if(ret) {
        dev_err(dev, "fail to get adc_threshold_gnd_average\n");
        goto fail;
    }

    ret = of_property_read_u32(np, "adc_threshold_left_average", &pdata->adc_threshold_left_average);
    if (ret) {
        dev_err(dev, "fail to get adc_threshold_left_average\n");
        goto fail;
    }
